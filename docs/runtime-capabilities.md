...
| CNB curve and animation-clip schemas, and the compilation front ends | CNA-TS | the container, the Texture2D, SpriteFont, model, sound-effect, song and video schemas are projected and verified above; what remains -- the curve and animation-clip schemas, the model's morph targets and per-slot texture arrays, the bounded byte cursor, the primitive byte writer, the loader registry and the .cnj compile path -- is measured and unprojected |
