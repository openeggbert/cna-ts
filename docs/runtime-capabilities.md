...
| CNB's remaining tooling: the byte writer, the loader registry and the .cnj front ends | CNA-TS | the container and every runtime asset schema -- Texture2D, SpriteFont, model, sound effect, song, video, curve and animation clip -- are projected and verified above. What remains is build-time tooling rather than runtime surface: the primitive byte writer, the loader registry, the .cnj compile front ends, the path-taking importers, and the model's morph targets and per-slot texture arrays |
