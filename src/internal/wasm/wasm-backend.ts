...
const ROUTES = [
  ...
  // The rest of the post-process family. Each is a PostProcessPass, so the apply, the chain and
  // the frame context above already serve them; what each adds is its creation and its parameters.
  ...
  // CNA's own scalar oracles: the same arithmetic the shaders do, as pure functions. They are what
  // let a browser check a rendered texel against CNA's answer for that texel.
